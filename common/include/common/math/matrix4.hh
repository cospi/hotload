...
struct Matrix4 {
	...
	)
	{
		...
		elements[10] = 2.0f / (near_plane - far_plane);
		elements[11] = (near_plane + far_plane) / (near_plane - far_plane);
		elements[15] = 1.0f;
		return matrix;
	}
	...
	)
	{
		...
		const float near_minus_far_inv = 1.0f / (near_plane - far_plane);
		...
		elements[10] = (near_plane + far_plane) * near_minus_far_inv;
		elements[11] = near_plane * far_plane * 2.0f * near_minus_far_inv;
		elements[14] = -1.0f;
		elements[15] = 1.0f;
		return matrix;
	}
